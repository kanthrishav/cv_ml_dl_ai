# oakd_aruco_ar_depth_sprites_ui.py
# OAK-D Lite: RGB + aligned metric depth. ArUco-anchored AR with 3 sprites
# (DOG, BENCH, BOY) and on-window low-transparency UI:
#   - radio to select active sprite
#   - sliders for X/Y/Z (mm) and Scale
#   - "Fill" checkbox + "Toggle Fill" button
#
...
Z_RANGE_MM = 80  # slider range for z (base height) in mm
SCALE_MIN, SCALE_MAX = 0.35, 2.5

# Assets (PNG with transparency preferred). If missing -> wireframe placeholder.
ASSETS = {
    "DOG":   "assets/dog.png",
...
WINDOW_NAME = "OAK-D Lite: Marker-anchored AR (Sprites UI) + Depth"

# ======================= SPRITES MODEL =======================

def load_sprite(path):
    if os.path.exists(path):
        img = cv2.imread(path, cv2.IMREAD_UNCHANGED)  # BGRA if available
        if img is not None:
            if img.ndim == 2:
...
def default_sprites():
    # Each sprite is an upright billboard anchored at the marker plane (z=0 base).
    # base_height_mm is the nominal sprite height in mm (before scaling).
    return [
        dict(  # DOG (replaces cuboid)
            name="DOG",
            type="sprite",
            asset_path=ASSETS["DOG"],
            sprite=load_sprite(ASSETS["DOG"]),
            base_height_mm=30.0,    # change to taste
            pos_mm=(+12.0, -10.0, 0.0),  # base center relative to marker center
            rot_deg=(0.0, 0.0, 15.0),    # yaw/tilt if desired
            scale=1.0,
            filled=True,                 # filled -> draw textured sprite; off -> wireframe
            color=(40, 230, 60),
            thickness=2,
        ),
        dict(  # BENCH (replaces cylinder)
            name="BENCH",
            type="sprite",
            asset_path=ASSETS["BENCH"],
            sprite=load_sprite(ASSETS["BENCH"]),
            base_height_mm=22.0,
            pos_mm=(-14.0, +10.0, 0.0),
            rot_deg=(0.0, 0.0, -5.0),
            scale=1.0,
            filled=True,
            color=(0, 200, 255),
        ...
        dict(  # BOY (replaces sphere)
            name="BOY",
            type="sprite",
            asset_path=ASSETS["BOY"],
            sprite=load_sprite(ASSETS["BOY"]),
            base_height_mm=32.0,
            pos_mm=(+2.0, +2.0, 0.0),
            rot_deg=(0.0, 0.0, 0.0),
            scale=1.0,
...
def randomize_positions(sprites):
    for s in sprites:
...
def build_sprite_quad(height_mm, aspect, pos_mm, rot_deg):
    """
    Build the 3D quad (p0..p3) for an upright billboard of given height and width=height*aspect.
    Base sits on marker plane (z=0). Local quad ordering: bottom-left, bottom-right, top-right, top-left.
    """
    h = float(height_mm)
    w = float(height_mm * aspect)
    x2 = w / 2.0
    # local coords before rotation: y=0 plane, bottom at z=0, top at z=-h
    p0 = np.array([-x2, 0.0,  0.0], dtype=np.float32)
    p1 = np.array([+x2, 0.0,  0.0], dtype=np.float32)
    p2 = np.array([+x2, 0.0, -h  ], dtype=np.float32)
    p3 = np.array([-x2, 0.0, -h  ], dtype=np.float32)
    quad_local = np.stack([p0,p1,p2,p3], axis=0)  # (4,3)

    R = euler_deg_to_R(*rot_deg, order="xyz")
    base_center = marker_center_offset() + np.array(pos_mm, dtype=np.float32)
    quad_world = (quad_local @ R.T) + base_center
    return quad_world  # (4,3)

def warp_sprite_onto(img_bgr, sprite_bgra, quad2d):
    """
    Perspective-warp a BGRA sprite onto polygon quad2d on img_bgr (in-place compositing).
    """
    h, w = img_bgr.shape[:2]
    ...
    src = np.array([[0,0],[sw-1,0],[sw-1,sh-1],[0,sh-1]], dtype=np.float32)
    dst = quad2d.astype(np.float32)

    H = cv2.getPerspectiveTransform(src, dst)

    # warp color and alpha to full canvas, then alpha-blend
    warped = cv2.warpPerspective(sprite_bgra, H, (w, h), flags=cv2.INTER_LINEAR, borderMode=cv2.BORDER_TRANSPARENT)
    color = warped[:,:,:3]
    alpha = warped[:,:,3:4].astype(np.float32) / 255.0

    # blend
    img_bgr[:] = (alpha * color + (1.0 - alpha) * img_bgr).astype(np.uint8)

def draw_sprite_wireframe(img, quad2d, color=(0,255,0), thickness=2, label=None):
    q = quad2d.astype(int)
    for i in range(4):
        cv2.line(img, tuple(q[i]), tuple(q[(i+1)%4]), color, thickness, cv2.LINE_AA)
    if label:
        cx, cy = q.mean(axis=0).astype(int)
        cv2.putText(img, label, (cx-20, cy-10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, color, 2)
...
class Button:
    def __init__(self, label, x, y, w, h):
        self.label = label; self.x=x; self.y=y; self.w=w; self.h=h
    ...
    def draw(self, img):
        ...
        cv2.addWeighted(overlay, 0.35, img, 0.65, 0, dst=img)
        cv2.rectangle(img, (self.x, self.y), (self.x+self.w, self.y+self.h), (120,220,255), 1)
        cv2.putText(img, self.label, (self.x+10, self.y+self.h-8),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.55, (240,240,240), 1, cv2.LINE_AA)
...
# Global UI state
UI = {
    "active_idx": 0,  # 0=DOG, 1=BENCH, 2=BOY
    "sliders": {},
    "fill_cb": None,
    "toggle_btn": None,
    "radio": None,
    "dragging": None,
    "ui_top": 120,  # moved lower so it doesn't overlap FPS text
}
...
def ui_build_controls(img_w, img_h):
    ...
    radio = RadioGroup(["DOG","BENCH","BOY"], left, top, w_each=130, h=28, active_idx=0)
    y = top + 42
    sliders = {
        "X (mm)": Slider("X (mm)", left, y, 360, 32, vmin=-RANDOM_RANGE_XY_MM*1.8, vmax=+RANDOM_RANGE_XY_MM*1.8, v0=0.0),
        "Y (mm)": Slider("Y (mm)", left, y+40, 360, 32, vmin=-RANDOM_RANGE_XY_MM*1.8, vmax=+RANDOM_RANGE_XY_MM*1.8, v0=0.0),
    ...
    cb = CheckBox("Fill (semi-transparent image)", left, y+170, 22, checked=True)
    btn = Button("Toggle Fill", left+220, y+166, 140, 30)
...
def ui_sync_from_sprite(sprite):
    UI["sliders"]["X (mm)"].value = sprite["pos_mm"][0]
    UI["sliders"]["Y (mm)"].value = sprite["pos_mm"][1]
    UI["sliders"]["Z (mm)"].value = sprite["pos_mm"][2]
    UI["sliders"]["Scale"].value  = sprite.get("scale", 1.0)
    UI["fill_cb"].checked         = sprite.get("filled", True)

def ui_apply_to_sprite(sprite):
    ...
    sprite["pos_mm"] = (x, y, z)
    sprite["scale"]  = sc
    sprite["filled"] = UI["fill_cb"].checked
    return sprite
...
def ui_mouse(event, x, y, flags, param):
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if idx is not None:
            UI["active_idx"] = idx
            UI["radio"].active = idx
            ui_sync_from_sprite(SPRITES[idx])
            return
        if UI["toggle_btn"].hit(x, y):
            SPRITES[UI["active_idx"]]["filled"] = not SPRITES[UI["active_idx"]]["filled"]
            UI["fill_cb"].checked = SPRITES[UI["active_idx"]]["filled"]
            return
        if UI["fill_cb"].hit(x, y):
            UI["fill_cb"].toggle()
            SPRITES[UI["active_idx"]]["filled"] = UI["fill_cb"].checked
            return
...
def main():
    ...
    with dai.Device(pipeline) as device:
        ...
        ui_sync_from_sprite(SPRITES[UI["active_idx"]])
        cv2.setMouseCallback(WINDOW_NAME, ui_mouse)

        template_id = None
        fps_smooth, t_prev = None, time.time()

        print("[INFO] Running. Mouse: radio/slider/checkbox/button on left pane. Keys: t=template, r=randomize, q=quit")
        while True:
            in_rgb = q_rgb.get()
            ...
            if (ids is not None) and (anchor_idx is not None):
                ...
                # Sync UI -> active sprite parameters
                SPRITES[UI["active_idx"]] = ui_apply_to_sprite(SPRITES[UI["active_idx"]])
                ...
                for s in SPRITES:
                    sc = s.get("scale", 1.0)
                    # compute aspect from asset if available, else 2:3 fall-back
                    if s["sprite"] is not None:
                        sh, sw = s["sprite"].shape[:2]
                        aspect = float(sw) / float(sh + 1e-9)
                    else:
                        aspect = 0.66  # generic portrait-ish placeholder

                    height_mm = s["base_height_mm"] * sc
                    quad3d = build_sprite_quad(height_mm, aspect, s["pos_mm"], s["rot_deg"])
                    quad2d = project_points(quad3d, rvec, tvec, K, dist)

                    if s.get("filled", True) and s["sprite"] is not None:
                        warp_sprite_onto(frame_bgr, s["sprite"], quad2d)
                        # Optional thin outline to make edges crisp
                        draw_sprite_wireframe(frame_bgr, quad2d, color=(0,0,0), thickness=1)
                    else:
                        draw_sprite_wireframe(frame_bgr, quad2d, color=s.get("color",(0,255,0)),
                                              thickness=s.get("thickness",2), label=s["name"])
            ...
            cv2.putText(stacked, "Mouse: select sprite, drag sliders, click 'Toggle Fill' | Keys: t=template, r=randomize, q=quit",
                        (20, 70), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (200,255,0), 2)

            # Draw UI on the left pane only (already placed lower to avoid overlap)
            ui_draw(stacked)
            ...
            if key == ord('q'):
                ...
                randomize_positions(SPRITES)
                ui_sync_from_sprite(SPRITES[UI["active_idx"]])
                print("[INFO] Sprites re-randomized around marker center.")
