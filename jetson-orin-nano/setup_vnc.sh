...
sudo apt-get install -y zstd xz-utils binutils sed grep coreutils lightdm-gtk-greeter || true
...
# Keep/create headless Xorg config for Jetson; do not delete it here
sudo rm -f /etc/X11/xorg.conf.d/20-headless.conf.template 2>/dev/null || true
...
sudo systemctl unmask lightdm.service display-manager.service 2>/dev/null || true
sudo systemctl set-default graphical.target
sudo groupadd -f autologin
sudo groupadd -f nopasswdlogin
sudo usermod -aG autologin,nopasswdlogin "$(id -un)"
SESSION_NAME="$( for s in ubuntu-xorg ubuntu gnome-xorg gnome; do [ -f "/usr/share/xsessions/${s}.desktop" ] && echo "$s" && break; done )"
...
EOF
sudo cp -a /etc/lightdm/lightdm-gtk-greeter.conf /etc/lightdm/lightdm-gtk-greeter.conf.bak 2>/dev/null || true
...
echo "== 3.5) Ensure Jetson has a real headless Xorg screen =="
sudo mkdir -p /etc/X11/xorg.conf.d
sudo tee /etc/X11/xorg.conf.d/20-headless.conf >/dev/null <<'EOF'
Section "Device"
    Identifier "Tegra0"
    Driver "nvidia"
...
echo "== 4) Ensure TigerVNC scraping server is installed (real :0 sharing) =="
if ! command -v x0tigervncserver >/dev/null 2>&1 && ! command -v x0vncserver >/dev/null 2>&1; then
  if ! sudo apt-get install -y tigervnc-scraping-server tigervnc-common; then
    sudo apt-get -y -d install tigervnc-scraping-server tigervnc-common || true
...
ExecStartPre=/bin/bash -lc 'for i in {1..90}; do [ -S /tmp/.X11-unix/X0 ] && { [ -f /run/lightdm/root/:0 ] || [ -f /var/run/lightdm/root/:0 ]; } && [ -x /usr/bin/x0tigervncserver ] && exit 0; sleep 1; done; echo "X0 socket, LightDM auth, or /usr/bin/x0tigervncserver not found"; exit 1'
ExecStart=/bin/bash -lc 'export HOME=/root USER=root LOGNAME=root; mkdir -p /root/.vnc; export XAUTHORITY=/run/lightdm/root/:0; [ -f "$XAUTHORITY" ] || export XAUTHORITY=/var/run/lightdm/root/:0; exec /usr/bin/x0tigervncserver -fg -display :0 -rfbport 5900 -PasswordFile /etc/x0vnc.pass -AlwaysShared -SecurityTypes VncAuth -localhost no'
Restart=on-failure
RestartSec=2
...
sleep 3
systemctl status x0vncserver.service --no-pager -l
xrandr --display :0 2>/dev/null || true
