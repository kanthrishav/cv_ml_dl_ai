...
# >>> 3D point-cloud viz >>>
# kept separate so nothing else changes
PC_STEP   = 4        # subsample pixels to keep it light on RPi
PC_FOCAL  = 300.0    # pseudo focal length for projection (pixels)
PC_ZBIAS  = 1.5      # push point cloud forward to avoid div-by-zero
PC_ROT    = 0.6      # fixed yaw-ish rotation in radians
# <<< 3D point-cloud viz <<<
...
def main_display(picam):
    ...
    while True:
        ...
        # >>> 3D point-cloud viz >>>
        # Build a simple projected 3D scatter from the normalized depth.
        pc_img = np.zeros_like(dov)  # same HxWx3
        if dov.size != 0 and dn.size != 0:
            step = PC_STEP
            yy, xx = np.mgrid[0:h:step, 0:w:step]  # subsample grid
            z = dn[yy, xx]                         # [0..1], nearer ~ smaller or larger depending on MiDaS range
            # Normalize coordinates to [-0.5, 0.5]
            X = (xx - w/2) / float(w)
            Y = (yy - h/2) / float(h)
            Z = 1.0 - z                             # flip so nearer pops out

            # Fixed yaw-like rotation for a 3D feel
            ca, sa = np.cos(PC_ROT), np.sin(PC_ROT)
            Xr = X*ca + Z*sa
            Zr = -X*sa + Z*ca
            Yr = Y
            ...
            denom = (Zr + PC_ZBIAS)
            xp = (Xr / denom) * PC_FOCAL + (w/2)
            yp = (Yr / denom) * PC_FOCAL + (h/2)

            m = (xp>=0) & (xp<w) & (yp>=0) & (yp<h) & np.isfinite(xp) & np.isfinite(yp)
            xp_i = xp[m].astype(np.int32).ravel()
            yp_i = yp[m].astype(np.int32).ravel()
            cols = dov[yy, xx][m].reshape(-1,3)

            pc_img[yp_i, xp_i] = cols
        # <<< 3D point-cloud viz <<<

        out = np.hstack((vis, dov, pc_img))
        cv2.imshow("Live | Depth+Detect+Binning | 3D", out)
